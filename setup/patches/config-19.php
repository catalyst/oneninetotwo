...
if (empty($_ENV['BACKUPID'])) {
    die('No backup ID supplied');
}
$backupid = $_ENV['BACKUPID'];



$CFG->dbtype    = 'postgres7';       // mysql or postgres7 (for now)
$CFG->dbhost    = '%dbhost%:%dbport%';   // eg localhost or db.isp.com
$CFG->dbname    = '%dbname%_'.$backupid;      // database name, eg moodle
...
$CFG->dataroot  = '%datadir%/backup-'.$backupid;
